...
public class PracticaParser {
    ...
    public static void main(String[] args) {
        ...
        parsear(String separador, String ignorar, String texto);
    ...
    private static List<String> parsear(String separador, String ignorar, String texto){
        List<String> tokens = new ArrayList<String>();
        int startPosition = 0;
        boolean isInQuotes = false;
        for (int currentPosition = 0; currentPosition < texto.length(); currentPosition++) {
            if (texto.charAt(currentPosition) == '\"') {
                isInQuotes = !isInQuotes;
            }
            else if (texto.charAt(currentPosition) == ',' && !isInQuotes) {
